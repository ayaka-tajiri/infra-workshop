resource "aws_iam_role" "eks" {
  name               = "OharuAmazonEKSRole"
  assume_role_policy = file("policies/eks-trust.json")
}

# 他の人のロールのアクセス権限をもらえる。Resourceが0になってることは全部のロールが対象？
resource "aws_iam_policy" "eks-sts" {
  name        = "OharuEKSStsAssumeRolePolicy"
  path        = "/"
...
resource "aws_iam_role" "eks-node-instance" {
  name               = "OharuNodeInstanceRole"
  # この子からなら接続してもOKですよ、みたいな情報がある
